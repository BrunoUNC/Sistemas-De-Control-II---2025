...
% Incluye cambio de referencia de desplazamiento de 10m a 0m luego de
% aumentar carga a 1Kg.
...
T =100;      % Tiempo total de simulación [s]
...
for k = 1:n-1
    % Cambio de condiciones: a los 15s cambia la masa y cambia la referencia a 0
    if t(k) >= 15 && m == 0.1
        m = 1;                %Cambio de referencia y carga para simular 
        reference(k:end) = 0; %grúa volviendo con una carga de 1Kg 
        % Recalcular matrices del sistema con nueva masa
        A = [0 1 0 0;
             0 -F/M -m*g/M 0;
             0 0 0 1;
             0 -F/(l*M) -g*(m+M)/(l*M) 0];
    ...
    % Zona muerta en accion de control
    %if abs(u_k) < 0.5, u_k = 0; else, u_k = sign(u_k) * (abs(u_k) - 0.5); end
    
    %if abs(u_hat_k) < 0.5, u_hat_k = 0; else, u_hat_k = sign(u_hat_k) * (abs(u_hat_k) - 0.5); end
...
figure;
subplot(3,1,1);
plot(t, delta, 'b', t, delta_hat, 'r--', t, reference, 'k-.'); grid on;
title('Desplazamiento \delta'); ylabel('[m]'); legend('Real','Estimado','Referencia');
